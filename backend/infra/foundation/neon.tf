...
resource "neon_project" "main" {
  name      = "${module.global.project_base_name}-${module.global.project_variant}${module.environment.resource_name_suffix}"
...
# JDBC connection string for the default branch/database/role, on the direct
# (non-pooled) endpoint (`database_host`, not `database_host_pooler`). The Lambda opens
# one short-lived connection per request at low volume, so it stays well within Neon's
# direct-connection limit and gains nothing from PgBouncer.
#
